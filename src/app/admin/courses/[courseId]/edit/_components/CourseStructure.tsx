...
function CourseStructure({ data }: iAppProps) {
  ...
  return (
<div ref={setNodeRef} style={style} {...attributes} className={cn("touch-none",  props.className, isDragging ? "z-10" : "")} >
      {children(listeners)}
    </div>
  );
}
  ...
  function handleDragEnd(event: DragEndEvent ) {
    ...
    if(activeType === "chapter"){
      ...
      const updateChapterForState = reorderLocalChapters.map((chapter, index) => ({
        ...chapter,
        position: index + 1,
      }));
    ...
    if (activeType === "lesson" && overType === "lesson"){
      ...
      const reorderedLessons = arrayMove(chapterToUpdate.lesson, oldLessonIndex, newLessonIndex);
                              ...
                              {(lessonListeners) => (
                                <div className='flex items-center justify-between p-2 hover:bg:accent hover:accent-hover rounded-md'>
                                  <div className='flex items-center gap-2'>
                                    ...
                                    <Button variant={"ghost"} size={"icon"} className='cursor-grab opacity-60 hover:opacity-100' {...lessonListeners}>
                                      <GripVertical className='size-4' />
                                    </Button>
